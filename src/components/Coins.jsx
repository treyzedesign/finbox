...
const Coins = () => {
  ...
  const market = ctx.coinCapData.map((item, index)=>{
    return<><tr key={index} className="text-sm font-semibold text-gray-700 px-4 py-12 h-20">
      <td >{item.cmc_rank}</td>
      <td className='py-7 flex justify-start pl-6'><img style={{display:"inline",height:"22px", width:"22px"}} src={`https://s2.coinmarketcap.com/static/img/coins/64x64/${item.id}.png`}/><span className='inline pl-3'>{item.name} <span className='text-gray-400'>{item.symbol}</span></span></td>
  ...
  return (
    ...
    <div className='relative '>
        <div className='flex justify-center position-relative   p-16 bg-gradient-to-br from-neutral-100 via-lime-200 to-lime-100 shadow-lg' style={{minHeight:'100vh'}}> 
        </div>
    ...
    <div className='my-custom-2' style={{height:"60vh"}}>
      <div className='flex items-center h-full justify-center text-gray-300'>
        <div className='text-center w-7/12'>
          ...
          <div className='my-2'>
            <input type='email' className=" flex-1 border rounded-md bg-grey-300 w-6/12 mx-4 mt-3  py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 focus:border-lime-600 sm:text-sm sm:leading-6" placeholder='example@gmail.com'/>
